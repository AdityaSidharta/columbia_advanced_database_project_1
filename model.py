...
def rocchio(query_vector, relevant_vectors, nonrelevant_vectors):
    return params.ALPHA * query_vector + params.BETA * (1. / len(relevant_vectors)) * np.sum(relevant_vectors) - params.GAMMA * (1. /len(nonrelevant_vectors)) * np.sum(nonrelevant_vectors)
